...
KEY = 'key'
ALL_TOKENS = [EMPTY, AGENT, DRAWN, STAR, LEFT_ARROW, RIGHT_ARROW, KEY]
...
TOKEN_IMAGES = {
    ...
    KEY : plt.imread(get_asset_path('green_button.png')),
}
...
class KeyBeforeTheStar(GeneralizationGridGame):
    ...
    def transition(layout, action):
        ...
        if token == EMPTY:
            ...
            KeyBeforeTheStar.step_move_in_direction(new_layout, -1)
        ...
        KeyBeforeTheStar.finish_simulation(new_layout)

        return new_layout

    @staticmethod
    def compute_reward(layout0, action, layout1):
        return float(KeyBeforeTheStar.compute_done(layout1))
    ...
    def step_move_in_direction(layout, direction):
        ...
        has_key = not np.any(layout == KEY)
        neighbor_cell = layout[r, c + direction]

        if neighbor_cell in [EMPTY, KEY] or (neighbor_cell == STAR and has_key):
            next_r, next_c = r, c + direction

        elif neighbor_cell == DRAWN:
            upper_cell = layout[r - 1, c + direction]
            if upper_cell in [EMPTY, KEY] or (upper_cell == STAR and has_key):
                next_r, next_c = r - 1, c + direction
            else:
                return

        elif neighbor_cell == STAR and has_key:
            next_r, next_c = r, c + direction
        ...
        while True:
            ...
            for r in range(height-2, -1, -1):
                for c in range(width):
                    ...
                    if (token == AGENT or token == DRAWN or token == KEY) and (layout[r+1, c] == EMPTY):
                        layout[r, c] = EMPTY
                        layout[r+1, c] = token
...
def create_random_layout():
    ...
    key_r = agent_r
    key_c = agent_c + rng.randint(1, agent_dist_from_stairs + stairs_height + 1)

    layout[star_r, star_c] = STAR
    layout[key_r, key_c] = KEY
...
create_gym_envs(KeyBeforeTheStar, layouts, globals())
